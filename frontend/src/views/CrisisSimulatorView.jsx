        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className={clsx(
              ...
              <span>{isCrisisActive ? 'Emergency Shock Active' : 'Nominal Network State'}</span>
            </span>
          </div>
          <h1 className="text-xl sm:text-2xl font-display font-bold text-slate-900 dark:text-white mt-1.5 flex items-center gap-2">
            <span>Crisis & Outbreak Simulation Engine</span>
            <Flame className="w-6 h-6 text-rose-500 animate-pulse" />
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Stress-test regional healthcare supply chains under disaster shocks with automated multi-facility rebalancing
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {isCrisisActive && (
            <button
              ...
              disabled={resetting}
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl bg-rose-600 hover:bg-rose-500 text-white shadow-md transition"
            >
              <RotateCcw className={clsx("w-3.5 h-3.5", resetting && "animate-spin")} />
              <span>{resetting ? 'Restoring Baseline...' : 'Reset to Baseline'}</span>
            </button>
          )}

          <button
            onClick={refreshData}
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl bg-white dark:bg-brand-dark-card border border-slate-200 dark:border-brand-dark-border text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-brand-dark-surface transition shadow-xs"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-slate-900 dark:text-white text-xs sm:text-sm">
                Nominal Baseline Operational State
              </h4>
              <p className="text-slate-500 dark:text-slate-400 text-[11px]">
                No emergency disaster shock is active. Choose a scenario below to simulate an acute public health shock.
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-brand-dark-surface/40 border border-slate-200/60 dark:border-slate-800 space-y-1">
                  <span className="font-bold text-slate-800 dark:text-slate-200 block text-xs">
                    Clinical Briefing: {currentScenario.title}
                  </span>
                  <p className="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
                    {currentScenario.description}
                  </p>
                  <div className="pt-1 text-[10px] text-slate-400 flex items-center gap-3">
                    <span>Expected Casualty Velocity: <strong>{currentScenario.estimated_casualties}</strong></span>
                    <span>District: <strong>{currentScenario.affected_district}</strong></span>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between text-[11px] mb-1">
                    <span className="text-slate-500">Epidemic Surge Intensity:</span>
                    <span className="font-bold font-mono text-purple-600 dark:text-purple-400 text-sm">
                      {intensity.toFixed(1)}x Peak Surge
                    </span>
                  </div>
                  <input
                    type="range"
                  ...
                  <div className="flex justify-between text-[9px] text-slate-400 mt-0.5 font-mono">
                    <span>0.5x (Moderate)</span>
                    <span>1.0x (Standard Outbreak)</span>
                    <span>2.5x (Catastrophic Flash Shock)</span>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-bold text-xs text-slate-800 dark:text-slate-200 block">
                        Auto-Evaluate Gemini Multi-Facility Rebalancing Plans
                      </span>
                      <span className="text-[10px] text-slate-400 block">
                        Instantly dispatches autonomous agent to source donor stock outside disaster zone
                      </span>
                    </div>
                  </div>
                  <input
                ...
                <button
                  ...
                  disabled={triggering}
                  className={clsx(
                    "w-full py-3.5 px-4 rounded-xl font-bold text-xs text-white shadow-lg transition flex items-center justify-center gap-2",
                    triggering
                ...
                >
                  {triggering ? (
                    <>
                      ...
                      <span>Injecting Disaster Surge & Calculating Corridors...</span>
                    </>
                  ) : (
                    <>
                      <Flame className="w-4 h-4" />
                      <span>🚨 Trigger Regional Crisis Shock ({intensity}x)</span>
                    </>
                  )}
                </button>
              </div>
